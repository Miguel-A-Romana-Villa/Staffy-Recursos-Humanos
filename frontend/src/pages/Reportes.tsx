...
export function Reportes() {
  ...
  useEffect(() => {
    Promise.all([reportesApi.resumen(), reportesApi.pagos(), reportesApi.asistencias()])
    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-4 md:grid-cols-4">
        <Card>
          ...
          <p className="mt-2 text-2xl font-bold">{resumen?.total_empleados ?? 0}</p>
        </Card>
        <Card>
          <p className="text-sm text-slate-500">Pagos calculados</p>
          <p className="mt-2 text-2xl font-bold">{formatMoney(resumen?.total_pagos ?? 0)}</p>
        </Card>
        <Card>
          <p className="text-sm text-slate-500">Tardanzas</p>
          <p className="mt-2 text-2xl font-bold">{resumen?.total_tardanzas ?? 0}</p>
        </Card>
        <Card>
          <p className="text-sm text-slate-500">Faltas</p>
          <p className="mt-2 text-2xl font-bold">{resumen?.total_faltas ?? 0}</p>
        </Card>
      </div>
            ...
            <tbody>
              {pagos.map((pago) => (
                <tr className="border-t border-slate-100" key={pago.periodo}>
                  ...
                  <td className="px-3 py-3 font-semibold">{formatMoney(pago.total_pagado)}</td>
                </tr>
              ))}
            </tbody>
          </Table>
          ...
          <Table>
            ...
            <tbody>
              {asistencias.map((asistencia) => (
                <tr className="border-t border-slate-100" key={asistencia.empleado_codigo}>
                  <td className="px-3 py-3">
                    <p className="font-medium">{asistencia.empleado_nombre}</p>
                    <p className="text-xs text-slate-500">{asistencia.empleado_codigo}</p>
                  </td>
